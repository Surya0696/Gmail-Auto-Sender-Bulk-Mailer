...
function parseCSV(csvText) {
  if (!csvText || typeof csvText !== 'string') {
    return { recipients: [], headers: [], totalRows: 0, validRows: 0, invalidCount: 0, error: 'Empty or invalid CSV file' };
  }
  ...
  if (rows.length === 0) {
    return { recipients: [], headers: [], totalRows: 0, validRows: 0, invalidCount: 0, error: 'No data rows found in CSV' };
  }
  ...
  emailColIndex = rawHeaders.findIndex(h => h.toLowerCase() === 'email' || h.toLowerCase() === 'e-mail' || h.toLowerCase() === 'email address');
  ...
  if (emailColIndex === -1) {
    ...
    const firstRowHasEmail = rawHeaders.some(cell => validateEmailFormat(cell));
    if (firstRowHasEmail) {
      ...
      rawHeaders = rawHeaders.map((_, i) => (i === emailColIndex ? 'email' : (i === 1 ? 'name' : `column_${i + 1}`)));
    } else {
      // Check first data row to find which column looks like emails
      if (dataRows.length > 0) {
        const sampleRow = dataRows[0];
        emailColIndex = sampleRow.findIndex(cell => validateEmailFormat(cell));
      }
      if (emailColIndex === -1) {
        // Default to column 0
        emailColIndex = 0;
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const emailValue = rowObject['email'] || (row[emailColIndex] ? row[emailColIndex].trim() : '');
  ...
  return {
    ...
    invalidCount,
    duplicateCount,
    error: recipients.length === 0 ? 'No valid email addresses found in CSV' : null
  };
}
