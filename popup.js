...
async function initUI() {
  ...
  document.getElementById("delayPresetSelect").value =
    settings.delayPreset || "15-25";
  ...
  document.getElementById("retryCountInput").value =
    settings.retryCount !== undefined ? settings.retryCount : 1;
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", removeCsvFile);
  ...
  document
    ...
    .addEventListener("click", removeAttachmentFile);
  ...
  // Delay Preset Change
  document
    .getElementById("delayPresetSelect")
  ...
  document
    ...
    .addEventListener("click", resumeCampaign);
  ...
  document
    .getElementById("closePreviewModalBtn")
    .addEventListener("click", closePreviewModal);

  document
    .getElementById("testSendBtn")
    .addEventListener("click", openTestModal);
  document
    .getElementById("closeTestModalBtn")
    .addEventListener("click", closeTestModal);
  document
    .getElementById("cancelTestModalBtn")
    .addEventListener("click", closeTestModal);
  document
    .getElementById("confirmTestSendBtn")
    .addEventListener("click", confirmTestSend);

  // View Failed List Quick Button
  document
    .getElementById("viewFailedListBtn")
    .addEventListener("click", () => switchTab("logsTab"));

  // Logs Action Buttons
  document
    .getElementById("exportLogsBtn")
    .addEventListener("click", exportLogsToCsv);
  document
    .getElementById("exportFailedLogsBtn")
    .addEventListener("click", exportFailedLogsToCsv);
  document
    .getElementById("retryFailedBtn")
    .addEventListener("click", retryFailedEmails);
  document
    .getElementById("campaignRetryFailedBtn")
    .addEventListener("click", retryFailedEmails);

  document
    .getElementById("clearLogsBtn")
    .addEventListener("click", clearAllLogs);
  document
    .getElementById("logFilterSelect")
    .addEventListener("change", renderLogsUI);
...
function showFileInfo(fileName, validCount, invalidCount) {
  ...
  document.getElementById("fileMetrics").textContent =
    `${validCount} valid emails${invalidCount > 0 ? ` (${invalidCount} invalid skipped)` : ""}`;
}
...
function showAttachmentInfo(fileName, fileSize) {
  ...
  document.getElementById("attachmentSize").textContent =
...
function openPreviewModal() {
  ...
  const sampleRow =
    parsedRecipients.length > 0
      ? parsedRecipients[0].data || parsedRecipients[0]
      : { email: "demo@example.com", name: "John Doe" };
  ...
  const renderedBody = renderTemplate(
    body || "Hello {{name}},\n\nThis is a preview email body.",
    sampleRow,
    signature,
  );

  document.getElementById("previewTo").textContent =
    sampleRow.email || "demo@example.com";
  document.getElementById("previewSubject").textContent = renderedSubject;
  document.getElementById("previewAttachmentVal").textContent =
    currentAttachment
      ? `${currentAttachment.fileName} (${formatFileSize(currentAttachment.fileSize)})`
      : "None";
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      if (response && response.success) {
        ...
        showToast(
          `Test send failed: ${response ? response.error : "Unknown error"}`,
        );
      }
    },
  );
...
function updateStateUI(state) {
  ...
  const remaining = Math.max(
    0,
    total - (sent + failed + (state.skippedCount || 0)),
  );
  ...
  document.getElementById("currentRecipientText").textContent =
    state.currentRecipient || "None";
  document.getElementById("estTimeText").textContent =
    `Est. Time: ${state.estimatedTimeRemaining || "--:--"}`;
  ...
  const progressPercent =
    total > 0
      ? Math.min(
          100,
          Math.round(
            ((sent + failed + (state.skippedCount || 0)) / total) * 100,
          ),
        )
      : 0;
...
async function renderFailedMailListUI() {
  ...
  if (failedLogs.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="3" class="empty-log">No failed emails in this campaign.</td></tr>';
    return;
  }
...
async function exportFailedLogsToCsv() {
  ...
  const failedLogs = logs.filter(
    (l) => l.status === "failed" && validateEmail(l.recipient),
  );
...
async function retryFailedEmails() {
  ...
  const failedEmailsSet = new Set(
    failedLogs.map((l) => l.recipient.toLowerCase()),
  );

  let retryList = parsedRecipients.filter((r) =>
    failedEmailsSet.has(r.email.toLowerCase()),
  );
  ...
  showToast(
    `Loaded ${parsedRecipients.length} failed emails for retry. Click Start!`,
  );
}
